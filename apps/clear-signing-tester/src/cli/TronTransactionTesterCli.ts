...
import { type TestTronTransactionUseCase } from "@root/src/application/usecases/TestTronTransactionUseCase";
...
export type TronCliConfig = {
  ...
  appTronVersion?: SpeculosConfig["version"];
...
/**
 * CLI for Tron transaction testing.
 * Composition root for the Tron clear-signing tester.
 */
export class TronTransactionTesterCli {
  private container: Container;
  private controller: ServiceController;
  private logger: LoggerPublisherService;
  private config: TronCliConfig;

  constructor(config: TronCliConfig) {
    this.config = config;
    ...
    this.container = makeTronContainer({
      config: diConfig,
      logger: {
        cli: {
  ...
  static createProgram(): Command {
    ...
    let cli: TronTransactionTesterCli | null = null;
    let exitCode = 0;

    program
      .name("tron-clear-signing-tester")
      .description("Tron Transaction Tester CLI")
      .version("1.0.0");

    program
      .option(
        "--derivation-path <path>",
        "Derivation path (default: \"44'/195'/0'/0/0\")",
        "44'/195'/0'/0/0",
      )
      .option(
        "--speculos-url <url>",
      ...
      .option(
        "--app-tron-version <version>",
        "Tron app version (e.g., 1.0.0). If not specified, uses latest version for the device.",
      )
      .option(
        "--os-version <version>",
    ...
    program.hook("preAction", async (_, command) => {
      const config = command.parent!.opts() as TronCliConfig;
      config.onlySpeculos = command.name() === "start-speculos";
      cli = new TronTransactionTesterCli(config);
    ...
    program
      ...
      .description("Test a single Tron raw_data transaction (hex-encoded)")
    ...
    program
      ...
      .description("Test multiple Tron raw_data transactions from a JSON file")
      .action(async (file) => {
  ...
  async handleRawTransaction(transaction: string): Promise<number> {
    const testUseCase = this.container.get<TestTronTransactionUseCase>(
      TYPES.TestTronTransactionUseCase,
    );
    ...
    const result = await testUseCase.execute(
      {
        ...
        rawTx: transaction,
        description: "Single Tron transaction test",
      },
  ...
  async handleRawFile(file: string): Promise<number> {
    const batchUseCase =
      this.container.get<TestBatchTransactionFromFileUseCase>(
        TYPES.TestBatchTronTransactionFromFileUseCase,
      );
...
async function main() {
  try {
    const program = TronTransactionTesterCli.createProgram();
